...
def train(  # pylint: disable=too-many-arguments
    ...
    for _ in range(epochs):
        for batch in trainloader:
            ...
            local_probs = F.log_softmax(local_logits / 3, dim=1)
            with torch.no_grad():
                global_probs = torch.softmax(global_logits / 3, dim=1)
            loss = criterion(local_logits, labels) + 9 * KLDiv(
                local_probs, global_probs
            )
            loss.backward()
            optimizer.step()
