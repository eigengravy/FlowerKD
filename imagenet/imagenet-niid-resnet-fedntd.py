...
def train(  # pylint: disable=too-many-arguments
    ...
    global_net = ResNet(ResidualBlock, [2, 2, 2, 2]).to(device)
    # Net(num_classes).to(device=device)
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def __init__(self, trainloader, valloader) -> None:
        ...
        self.model = ResNet(ResidualBlock, [2, 2, 2, 2])  # Net(num_classes=200)
...
def get_evaluate_fn(
    ...
    def evaluate_fn(
        ...
        model = ResNet(ResidualBlock, [2, 2, 2, 2])  # Net(num_classes=200)
...
def main() -> None:
    ...
    save_path = "outputs/imagenet-niid-resnet-" + datetime.now().strftime("%d-%m-%H-%M")
