...
class Net(nn.Module):
    def __init__(self, num_classes=200):
        ...
        #     nn.MaxPool2d(kernel_size=2, stride=2),
        # )
        # self.fc = nn.Sequential(nn.Dropout(0.5), nn.Linear(2048, 2048), nn.ReLU())
        # self.fc1 = nn.Sequential(nn.Dropout(0.5), nn.Linear(2048, 512), nn.ReLU())
        # self.fc2 = nn.Sequential(nn.Linear(512, num_classes))

    def forward(self, x):
        x = F.relu(self.batch_norm1(self.conv1(x)))
        x = self.pool(F.relu(self.batch_norm2(self.conv2(x))), 2, 2)
        x = F.relu(self.batch_norm3(self.conv3(x)))
        x = self.pool(F.relu(self.batch_norm4(self.conv4(x))), 2, 2)
        x = F.relu(self.batch_norm5(self.conv5(x)))
        x = F.relu(self.batch_norm6(self.conv6(x)))
        x = self.pool(F.relu(self.batch_norm7(self.conv7(x))), 2, 2)
